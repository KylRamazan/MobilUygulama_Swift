...
class UniversitySearchViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(self.aramaYapiliyorMu) {
            cell.textLabel?.text = aramaSonucu[indexPath.row].okulAdi
            cell.detailTextLabel?.text = aramaSonucu[indexPath.row].adSoyad
        }else{
            cell.textLabel?.text = verilerUni[indexPath.row].okulAdi
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if(searchText == "") {
            ...
            self.aramaSonucu = self.verilerUni.filter({$0.okulAdi.lowercased().contains(searchText.lowercased())})
        }
